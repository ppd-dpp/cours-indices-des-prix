...
& # 128214; Manuel RPPI: chapitres 5 à 6.

& # 128214; Manuel PPI: Chapitre 21, sections A, C, D.
...
\ begin {align *}
\ log (I ^ {Q}) = & \ underbrace {E (\ rho | t = 1) - E (\ rho | t = 0)} _ {\ text {indice des prix de transaction}} \\
& + \ underbrace {[E (X | t = 0) - E (X | t = 1)] [\ beta_ {0} P (t = 1) + \ beta_ {1} P (t = 0)]} _ {\ text {terme de correction}}.
\ end {align *}
...
\ begin {align *}
\ omega_ {x} = \ frac {\ text {var} (t | X = x) P (X = x)} {\ sum_ {x} \ text {var} (t | X = x) P (X = X)}.
\ end {align *}
...
regroupé <- lm (log (prix) ~ période, df)
exp (coef (groupé) [2]) * 100
`` ''
...
hedonic_imputation <- lm (log (prix) ~ période + foie + saumon +
                           période: (I (foie - moyenne (foie)) + I (saumon - moyenne (saumon))),
                         df)
...
hi0 <- lm (log (prix) ~ foie + saumon, df, sous-ensemble = période == 0)

# Régression de la période 1
hi1 <- lm (log (prix) ~ foie + saumon, df, sous-ensemble = période == 1)

# Calculer l'indice à l'aide des prix prévus
exp (moyenne (prédire (hi1, df) - prédire (hi0, df))) * 100

`` ''

C'est également la même chose que le calcul manuel de l'indice stratifié.
`` `{r}
# Apportez dans la bibliothèque ppd
bibliothèque (ppd)

# Poids de la nourriture pour chats par sa fréquence
poids <- colSums (df [- (1: 2)])

# Calculer un indice pour chaque type de nourriture pour chat
strata_indices <-
  sapply (list (sous-ensemble (df, chicken == 1),
              sous-ensemble (df, foie == 1),
              sous-ensemble (df, saumon == 1)),
         fonction (x) avec (x, géomée (prix [période == 1]) / géomée (prix [période == 0]))
         )

# Agrégat
géomée (strata_indices, poids) * 100
`` ''

Les résultats sont très similaires si un modèle temporel est utilisé à la place.
`` `{r}
# Indice temporel factice
time_dummy <- lm (log (prix) ~ période + foie + saumon, df)
...
`` `{r}
# Faites des poids
poids df $ <- 1:25 / somme (1:25)

# Calculer l'indice d'imputation hédonique pondéré
hedonic_imputation <- lm (log (prix) ~ période + foie + saumon +
                           période: (I (foie - moyenne pondérée (foie, poids)) +
                                   I (saumon - poids pondéré (saumon, poids))),
                         df, poids = poids)
exp (coef (hedonic_imputation) [2]) * 100
`` ''
...
`` `{r}
# Régression période 0
hi0 <- lm (log (prix) ~ foie + saumon, df, sous-ensemble = période == 0, poids = poids)

# Régression de la période 1
hi1 <- lm (log (prix) ~ foie + saumon, df, sous-ensemble = période == 1, poids = poids)

# Calculer l'indice à l'aide de prix prédits pondérés
exp (weighted.mean (predire (hi1, df) - predire (hi0, df), df $ poids)) * 100
...
Les quatre questions suivantes concernent le type d'ajustement de qualité suivant. Une stratégie courante pour construire un indice de qualité constante lorsque l'emballage d'un produit change consiste à diviser le prix par la taille de l'emballage (par exemple, le prix par litre de savon à vaisselle), de sorte que le prix relatif du produit $ i $ devienne $ p_ {i1} / p_ {i0} \ fois s_ {i0} / s_ {i1} $, où $ s_ {it} $ est la taille du paquet dans la période $ t $. Un indice basé sur cette approche peut être calculé avec une régression linéaire, comme
...
Les deux questions suivantes portent sur une variation de l'indice hédonique factice en fonction des médianes conditionnelles, de sorte que $ \ text {med} (\ rho | X, t) = \ alpha_ {0} + t (\ alpha_ {1} - \ alpha_ {0}) + X \ beta $. Avec l'indépendance conditionnelle, $ \ alpha_ {1} - \ alpha_ {0} = \ log (\ text {med} (p (1)) / \ text {med} (p (0))) $.
...
\ begin {align *}
\ underbrace {E (\ rho | t = 1) - E (\ rho | t = 0)} _ {\ text {indice des prix de transaction}} = & \ underbrace {E (\ rho (1) | t = 0 ) - E (\ rho (0) | t = 0)} _ {\ text {Indice de qualité constante semblable à Laspeyres}} \\
& + \ underbrace {E (\ rho (1) | t = 1) - E (\ rho (1) | t = 0)} _ {\ text {biais de sélection}}.
\ end {align *}
